...
namespace ChartAndGraph
{
    ...
    public class BarData : AbstractChartData,IInternalBarData, IChartData
    {
        ...
        public ChartDynamicMaterial GetCategoryMaterial(string name)
        {
            var cat = mCategories.FirstOrDefault(c => c.Name == name);
            if (cat != null)
            {
        ...
        public object[] StoreAllCategories()
        {
            //Todo: implement a vieworder for the DataEntry class
            return mData.Cast<object>().ToArray();
            //return mData.Where(x => x.ViewOrder >= 0).OrderBy(x => x.ViewOrder).Cast<object>().ToArray();
        }
